...
GenerateCategoricalFDTest <- function(klen, mu1_coef,mu2_coef,num_indvs, timeseries_length,
                                      time_interval, fl_choice, lp_intercept, flparam){
  ...
  flfn <- switch(fl_choice,
                 ...
                 "30"=list("fl1"=rep(-0.2,timeseries_length),
                          "fl2"=matrix(flparam[1] + flparam[2]*sin(2*pi*time_interval) + flparam[3]*cos(2*pi*time_interval),nrow=timeseries_length,ncol=1),
                          "fl3"=matrix(-20*sin((2*pi/25)*(time_interval-1))-6,nrow=timeseries_length,ncol=1))
  )
...
calc_integral_penalty <- function(timeseries_length, timestamps01, results){
  flc2 <- results$flfn2
...
my_fitness <- function(mu1_coef, mu2_coef, intercept, flc, flparam){
  ...
  int_pen <- calc_integral_penalty(timeseries_length, timestamps01, results)
  sq_mean_val <- int_pen$sq_mean_val
  var_penalty <- int_pen$var_penalty
  
  if(all(flparam[1:3] == 0)){
    sq_mean_val <- 1000.0
    var_penalty <- 1000.0
  }
  ...
  # minimizing_fitness <- cbind(balance_penalty, distance_penalty, sq_mean_val, var_penalty)
  
  minimizing_fitness <- cbind(sq_mean_val, var_penalty)
